...
float filter1(float Input_Data)//runs every 23 microseconds
{
    int i;
    float sum = 0;
    static int oldest = 0;
   ...
   // The oldest sample in the circular buffer is overwritten.
    filtbuff1[oldest] = Input_Data; //first data will only be overwritten after 127 iterations

    for ( i = 0; i < FIR_FILTER_LENGTH; i++)
    {
         int j = (oldest - i + FIR_FILTER_LENGTH) % FIR_FILTER_LENGTH; // value accessed in reverse order since x[-k] has to happen
         sum += firCoeffs[i] * filtbuff1[j];
    }    //summation h[k]*x[n-k]
         oldest = (oldest + 1) % FIR_FILTER_LENGTH;
         //next position of input sequence is calculated(oldest sequence is replaced in circular buffer).

   // to be completed
    return sum;  // Return the filtered output
}
